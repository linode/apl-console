...
export function Breadcrumb(props: BreadcrumbProps) {
  ...
  } else if (removeCrumbX != null) pathMap = removeByIndex(allPaths, removeCrumbX - 1)
  else pathMap = allPaths

  const hasError = Boolean(onEditHandlers?.errorText)

  return (
    <StyledRootDiv sx={{ ...(hasError && { marginBottom: (theme) => theme.spacing(3) }) }} {...breadcrumbDataAttrs}>
      <StyledPreContainerDiv sx={{ ...(onEditHandlers !== undefined && { alignItems: 'center' }) }}>
        <Crumbs
