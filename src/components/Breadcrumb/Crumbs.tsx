...
import { StyledDiv, StyledSlashTypography, StyledTypography } from './Crumbs.styles'
...
export const Crumbs = React.memo((props: Props) => {
  ...
  const finalCrumbs = firstAndLastOnly && pathMap.length > 1 ? firstCrumb : allCrumbsButLast
  ...
  return (
    <>
      ...
      {labelOptions && labelOptions.prefixComponent && (
        <FinalCrumbPrefix prefixComponent={labelOptions.prefixComponent} prefixStyle={labelOptions.prefixStyle} />
      )}
    ...
    */}
      {labelOptions && labelOptions.suffixComponent && labelOptions.suffixComponent}
    </>
  )
})
