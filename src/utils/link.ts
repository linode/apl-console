 ...
 * @returns string
 */
export const flattenChildrenIntoAriaLabel = (children: React.ReactNode): string => {
  if (typeof children === 'string') return children
  if (React.isValidElement(children)) {
    // If children is a single React element, extract its text content if any.
    return children.props.children ? flattenChildrenIntoAriaLabel(children.props.children) : ''
  }
...
export const childrenContainsNoText = (children: React.ReactNode): boolean => {
  ...
  if (React.isValidElement(children)) {
    ...
    if (childText === null) return true // Consider null as having no text content.

    if (typeof childText === 'string') return childText.trim() === ''

    // Check if the element is a valid container (e.g., React.Fragment) and has no text content.
    if (React.isValidElement(childText) || Array.isArray(childText)) return childrenContainsNoText(childText)

    return true // Return true for other cases with non-string children.
  }
  if (Array.isArray(children)) return children.every((child) => childrenContainsNoText(child))
